...
import {getCommitteeData} from '../../lib/api'

import Axios from 'axios';
import CommitteeDetail from '../detail/CommitteeDetail';
import SpinningComponent from '../spin/SpinningComponent';
const { TabPane } = Tabs;
const  Committee = ({tabKey,routeTitle,menuData,cDetailData,routeUri,setTabKey}) => {
    ...
    React.useEffect(()=>{
        ...
        if(tabKey!==null){
          async function fetchData() {
            const cmData = await getCommitteeData(tabKey) //applo client 
                                ...
                                {mData!==null?mData.childItems.edges.map((t,idx)=>
                                      <TabPane tab={t.node.label} key={t.node.uri}>
                                        <div className='page-content'>
                                            ...
                                            {title == 'Committees' && crData!==null?<CommitteeDetail
                                            cDetailData={crData}/>:<SpinningComponent/>  
                                         }
                                        </div>
                                      </TabPane>
  ):<></>}
